...
public ResultSet SQLSelect1(String field , String table) throws ClassNotFoundException
{
    ResultSet rs = null ;
    try
    {
...
public String getPrimaryKey(String field,String table , String prefix) throws ClassNotFoundException
{
    ...
    try{
        ...
        if(result.size()>0)
        {
            current = Integer.parseInt(result.get(result.size()-1).toString().substring(4,7))+1;
...
public String getPrimaryKey2(String field,String table , String prefix) throws ClassNotFoundException
{
    ...
    try{
        ...
        if(result.size()>0)
        {
            current = Integer.parseInt(result.get(result.size()-1).toString().substring(4,10))+1;
           if(current>0 && current<=9){return prefix +"00000"+current;}
           else if(current>9 && current<=99){return prefix +"0000"+current;}
           else if(current>99 && current<=999){return prefix +"000"+current;}
           else if(current>999 && current<=9999){return prefix +"00"+current;}
           else if(current>9999 && current<=99999){return prefix +"0"+current;}
           else if(current>99999 && current<=999999){return prefix +current;}
          //else if(current>999999 && current<=9999999){return prefix +current;}
        }
    }catch(SQLException ex){//SQL Exception

        }
    return prefix+"000001";//Return
}
